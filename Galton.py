...
for i in range(Hist + Fut -1, nb_row-1):
    mean_hist  = []
    mean_fut = []
    var_hist  = [] #ca sera les std dedans
    var_fut = []
    for k in Name:
...
for i in range(Hist + Fut -1, nb_row-1):
    cor_hist  = []
    cor_fut = []
    Matrix_hist = df_rendement.iloc[i-Hist - Fut + 1:i -Fut+1].corr(min_periods= Hist)
    Matrix_fut = df_rendement.iloc[i- Fut+ 1: i+1].corr(min_periods=Fut)
...
#calcul covariance à partir de correlation
coef_p = df_rendement[Actif].iloc[:nb_row-1].tail(Hist).corr()
coef_p_corrige = df_rendement[Actif].iloc[:nb_row-1].tail(Hist).corr() #probleme lors des calculs si je fais coef_p_corrige = coef_p
...
#calcul covariance par correlation et risque 
...
print("matrice coef ;", coef_p)
print("matrice coef corrige ;", coef_p_corrige)

Cov = np.dot(np.diag(std_non_corrige), np.dot(coef_p, np.diag(std).T))
Cov_corrige = np.dot(np.diag(std), np.dot(coef_p_corrige, np.diag(std).T))
